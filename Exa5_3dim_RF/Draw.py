...
fig_file=fig_path+'/RF-testError.svg'
plt.savefig(fig_file, format='svg')
fig_file=fig_path+'/RF-testError.png'
...
if run_my:
    ax.plot(msePredStep[:draw_len], Loss_cs_mean[:draw_len], color=rgb_color_cs, label='ICODE', linewidth=2, marker='*', markevery=int(num_epochs/5))
if run_CDE:
    ax.plot(msePredStep[:draw_len], Loss_c_mean[:draw_len], color=rgb_color_c, label='CDE', marker='.', markevery=int(num_epochs/3))
if run_NeuralODE:
    ax.plot(msePredStep[:draw_len], Loss_n_mean[:draw_len], color=rgb_color_n, label='NODE', marker='+', markevery=int(num_epochs/4))
if run_AugNeuralODE:
    ax.plot(msePredStep[:draw_len], Loss_aug_mean[:draw_len], color=rgb_color_aug, label='ANODE', marker='x', markevery=int(num_epochs/6))
ax.set_xlabel('Epoch', fontsize=fontsize_xylabel)
...
pos=[0.5, 0.45]
y_ratio=50
# ZoomIn(ax, range(draw_len), Loss_cs_mean[:draw_len], Loss_c_mean[:draw_len], Loss_n_mean[:draw_len], Loss_aug_mean[:draw_len], width=width, height=height, pos=pos, draw_len=num_epochs, y_ratio = y_ratio)
fig_file=fig_path+'/RF-trainError.svg'
plt.savefig(fig_file, format='svg')
fig_file=fig_path+'/RF-trainError.png'
plt.savefig(fig_file, dpi=300)
plt.show()
