...
def ZoomIn(ax, msePredStep, msePred_cs_mean, msePred_c_mean, msePred_n_mean, msePred_aug_ode_mean,width=0.3, height=0.2, pos=[10, 3], draw_len=num_epochs, y_ratio = 60):

    axins = ax.inset_axes((pos[0], pos[1], width, height))
    axins.plot(msePredStep[:draw_len], msePred_cs_mean[:draw_len], color=rgb_color_cs, label='ICODE', linewidth=3, marker='*', markevery=10)
    ...
    ylim0_f = np.min(y)-(np.max(y)-np.min(y))*y_ratio*0.8
    ylim1_f = np.max(y)+(np.max(y)-np.min(y))*y_ratio*0.8
    ...
    #         axesA=axins,axesB=ax)
    # axins.add_artist(con)
    # ax.arrow((zone_left+zone_right)/2, np.max(y)+(np.max(y)-np.min(y))*y_ratio*2, -0.0000, 0.0000, width=0.03, zorder=2)
    # ax.arrow(90, 0.0001, -0.00001, 0.00001)