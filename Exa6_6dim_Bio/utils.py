...
# tSwitchPoint1=T_end*np.array(config['input']['tSwitchPoint1'])
# tSwitchLen1=T_end*config['input']['tSwitchLen1']
# xVal1=config['input']['xVal1']
...
class cl_u():
    def __init__(self, tSwitchPoint, tSwitchLen, xVal):
        self.tSwitchPoint=tSwitchPoint
        self.tSwitchLen=tSwitchLen
        self.xVal=xVal

    def __call__(self, t):
        if t<self.tSwitchPoint[0]:
            return self.xVal[0]
        count=-1
        for i,it in enumerate(self.tSwitchPoint):
            if t<it:
                break
            else:
                count+=1
        if t>=self.tSwitchPoint[count]+self.tSwitchLen:
            temp=(count+1)%2
            return self.xVal[temp]
        
        t0=self.tSwitchPoint[count]
        k=-2/self.tSwitchLen**3 * (t-t0)**3 + 3/self.tSwitchLen**2 * (t-t0)**2
        x0=self.xVal[count%2]
        x1=self.xVal[(count+1)%2]
        res=x0+k*(x1-x0)
        return res
