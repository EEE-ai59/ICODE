    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % num_epochs_show == 0:
            ...
            with torch.no_grad():
                ...
                for i in range(test_len):
                    mse += mean_squared_error(Z[:,0,i,:], sol_fine[:,train_len+i,:])
                if (epoch + 1) % 20*num_epochs_show == 0:
...
sol_fine0=sol_fine
noise=noise_p*(2*np.random.rand(sol_fine.shape[0], train_len, sol_fine.shape[2])-1)
sol_fine[:,:train_len,:]=np.multiply(noise, sol_fine[:,:train_len,:])+sol_fine[:,:train_len,:]
...
if run_my:
    ...
    for i in range(n_seq):
        R2_cs_ode += r2_score(Z_cs_ode[i,:,:], sol_fine[i,train_len:,:]) # 改方向
    R2_cs_ode /= n_seq
