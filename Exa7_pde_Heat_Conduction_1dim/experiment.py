...
train_len=int(len(tL_all)/3)*2-1
...
def my_u(t):
    if torch.is_tensor(t):
        # ans=-np.pi/10*torch.exp(-np.pi**2/10**2*t)
        ans=2*torch.sin(np.pi*2*t)*torch.exp(-t/5)+0.1
    else:
...
def my_du(t):
    if torch.is_tensor(t):
        ans=-(4*np.pi*torch.cos(2*np.pi*t) -2/5*torch.sin(2*np.pi*t) )*torch.exp(-t/5)
        ans=(4*np.pi*torch.cos(2*np.pi*t) -2/5*torch.sin(2*np.pi*t) )*torch.exp(-t/5)
    else:
        ans=-(4*np.pi*np.cos(2*np.pi*t) -2/5*np.sin(2*np.pi*t) )*np.exp(-t/5)
        ans=(4*np.pi*np.cos(2*np.pi*t) -2/5*np.sin(2*np.pi*t) )*np.exp(-t/5)
    return 0*ans
